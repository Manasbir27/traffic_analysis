...
while cap.isOpened():
    ...
    if len(detections):
        # Apply NMS
        keep = torch.ops.torchvision.nms(detections[:, :4], detections[:, 4], iou_threshold=0.5)
        ...
        for i, det in enumerate(detections):
            ...
            label = model.names[int(det[5])]  # Get the label name
            ...
            # Display label and speed (if applicable)
            text = label
            if label in vehicle_types and i in speed_measurements and len(speed_measurements[i]) > 0:
                avg_speed = np.mean(speed_measurements[i][-10:])  # Average of last 10 measurements
                text = f"{label}: {avg_speed:.2f} km/h"
